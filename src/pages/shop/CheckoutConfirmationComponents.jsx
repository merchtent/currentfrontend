...
const CheckoutSuccess = () => {
    ...
    // clear cart
    // clear the context paynment id

    // as stripe will validate if the payment intent has been used and was successfully paid. User cannot pay again, so if user has already paid, 
    // we need to redirect to order submissions
    ...
    useEffect(() => {
        ...
        stripe.retrievePaymentIntent(clientSecret).then(async ({ paymentIntent }) => {
            ...
            if (paymentIntent.status === 'succeeded') {
                // update backend order was paid
                const response = await post(`${urlOrders}/status`, statusUpdate);
            }
        });
    }, [stripe]);
    ...
    const fetchOrder = async () => {
        try {
            const response = await axios.get(`${urlOrders}/transactionid?id=${intentId}`);
        ...
        <>
            <Box sx={{
                ...
                >
                    ...
                    >
                        <Box>
                            <Typography level='title-lg'>YOUR ORDER IS HAPPENING</Typography>
                            <Typography level='body-sm'>An email confirmation has been sent to you!</Typography>
                            {/* <Typography level='title-md'>{STATUS_CONTENT_MAP[status].text}!</Typography> */}
                            {/* <Typography>Transation ID: {intentId}</Typography> */}
                        ...
                        {order &&
                            <>
                                <Box>
                                    ...
                                    <Typography level='body-sm'>Order Number: ML-{order.id}</Typography>
                                    <Typography level='title-md'>{order.firstname} {order.lastname}</Typography>
                                </Box>
                                {/* <Box sx={{ mt: 4 }}>
                                    <Typography level='title-md'>CHARITY DONATION</Typography>
                                    <Typography level='body-sm'>$1.50 - Sooki Lounge</Typography>
                                </Box>
                                <Box sx={{ mt: 4 }}>
                                    <Typography level='title-md'>MERCH CREDITS</Typography>
                                    <Typography level='body-sm'>You've earned 12 Credits</Typography>
                                </Box> */}
                            </>}
                    </Box>
                </Box>
